...
main(int argc,char *argv[])
{
	if(argc < 3)
	{
		printf("Arguments should have input filename and output filename");
		exit(0);
	}
	...
	printf("%s \n", input_file);
	printf("%s \n", output_file);
	// Foreach line in input_file, we need to check IsIsomprph with all lines in output file
	// If output file does not exist or isEmpty, we should create it and put the first line in input_file in it
	// Else, we read line from input file, check isomorphism with all lines in output and either ignore it 
	// or add it to the output_file it is not isomorphic with any of the lines (graphs) in output file
	
	int gsize = 99;
	int line_size = gsize*gsize+1;
	int * g = (int *)malloc(gsize*gsize*sizeof(int));
	if(g == NULL) {
		exit(1);
	}
	int * g1 = (int *)malloc(gsize*gsize*sizeof(int));
	if(g1 == NULL) {
		exit(1);
	}
    ...
    char *my_string;
  	my_string = (char *) malloc (line_size);
 
	printf("Before the main loop");
	while ( getline(&my_string,&line_size , in_file) != -1 )
	{
	 	int element_count = 0;
	 	for (element_count;element_count<gsize*gsize;element_count++)
		{
				if(my_string[element_count]=='0')
				{
					g[element_count] = 0;
				}
				else
		...
		out_file = fopen (output_file , "r");
	   	...
	   	int is_iso = 0;
		printf("before reading from output file \n");
	   	while ( getline(&my_string,&line_size , out_file) != -1 )
		{	
			int element_count = 0;
		 	for (element_count;element_count<gsize*gsize;element_count++)
			{
			...
			int result = IsIsomorph(g,g1,gsize);
			printf("after the check %d \n",result);
			if(result)
			{
				is_iso = 1;
				break;
		...
		// append to output file the new graph
		else
		{
